...
# [done]添加标题，X轴说明，Y轴说明，图例说明，统一为中英双语，数值显示， 色系统一风格
# [done] 直方图留一个
# [done]整个UI主题颜色换深色，高端
# [done]箱线图美化，略显简单，扩充随机模块
# [done]概率密度图美化，显示字符重叠问题解决
# [done] 小提琴图无法显示，提示 openpyxl库没有
# [done]六边形箱图，添加对应的实际物理含义
# [done]饼图美化，色系，图例叠加，拆分两个，单饼图，嵌套饼图
# [done]热力图显示函数自动关闭演示关闭
# [done]面积图换函数，换成多折线图，带比例占比显示
# [done]sankey图依赖web显示，加载慢问题解决，统一展示位置，而不是弹窗到web
# [done]地图，依赖函数传参，缺少mode参数调用失败，需要支持函数参数界面输入 
# [done]气泡图的数据看不懂， readme更新，每个图增加示例及数据集说明
# [done]集合图，打印了两次，串口有告警信息，色系黑白没有统一， 
# [done]词云图，换数据集，自定义数据集
# [done]维诺图，依赖函数传参，缺少mode参数调用失败，需要支持函数参数界面输入
# [done]金字塔图，依赖函数传参，缺少mode参数调用失败，需要支持函数参数界面输入
# [done]漏斗图，依赖web，统一显示风格
# [done]树图，连续打了两个，风格太丑
# [done]矩形树图有bug，随机了两个类型名称一样
# [done]人物关系图有bug，同一对人之间，随机出来有多条线，并且太丑
# [UNDO]动态排名图，GDP数据没转换，太长了
# [done]动态折线图，需要做多个折线，表示比较和增长，最好用一个硬盘，ssd的销量数据来做
# [UNDO]所有图的左上角UI有一个figure1，需要修改自定义
# [done]函数模块，依赖函数传参，缺少type参数导致失败
# [done]函数模块，最好是实现成用户可以自定义函数，然后，读取定义，然后生成曲线；
# [UNDO]动态图内容太少
# [done]函数图内容太少
# [done]终端的调试打印去除
